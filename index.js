...
// Load Vcard Contact
const vcard = 'BEGIN:VCARD\n' // metadados do cartão de contacto
            + 'VERSION:3.0\n' 
            + 'FN:デニス\n' // nome completo
            + 'ORG:Owner Dns Bot;\n' // a organização do contacto
            + 'TEL;type=CELL;type=VOICE;waid=6285866295942:+62 858-6629-5942\n' // WhatsApp ID + numero de celuar
            + 'END:VCARD'
prefix = '!'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'adicionar') {
				...
				teks = `Karai vc entrou @${num.split('@')[0]}\nBem vindo ah fcc *${mdata.subject}*`
				...
				teks = `Boa sorte mano @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isCmd = body.startsWith(prefix)
                        const tescuk = ["351965098553@s.whatsapp.net"]
                        const q = args.join(' ')

			mess = {
//DATABASE X
			}
    			const apakah = ['Sim','Náo']
        		const bisakah = [,'Lata',' Lata']
		        const kapankah = ['Mais dia','Mais domingo','Mais meses','Toutro ano']
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '351965098553@s.whatsapp.net'
                        const isEventon = isGroup ? event.includes(from) : false
                        const isRegister = checkRegisteredUser(sender)
                        pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefined

                        // Options
                        const botName = 'TSUNAMI BOT'
                        const ownerName = ''
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`*「 subir de nível 」*\n\n➸ *Nome*: ${sender}\n➸ *XP*: ${getLevelingXp(sender)}\n➸ *nivell*: ${getLevel} -> ${getLevelingLevel(sender)}\n\nParabéns!! 🎉🎉`)
                }
            } catch (err) {
                console.error(err)
            }
          ...
          const checkLimit = (sender) => {
                let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return client.sendMessage(from,`Seu limite de solicitação está completo habis\n\n_Note : O limite será zerado todas as vezes jam 21:00!_`, text,{ quoted: mek})
           ...
           return false
			...
			const isQuotedImage = type === 'mensagem de texto estendida' && content.includes('imageMessage')
			const isQuotedVideo = type === 'mensagem de texto estendida' && content.includes('videoMessage')
			const isQuotedSticker = type === 'memensagem de texto estendida' && content.includes('stickerMessage')
			if (!isGroup && isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'from', color(sender.split('@')[0]), 'args :', color(args.length))
			if (!isGroup && !isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mRECV\x1b[1;37m]', time, color('Menssagem'), 'from', color(sender.split('@')[0]), 'args :', color(args.length))
			if (isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'from', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			if (!isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mRECV\x1b[1;37m]', time, color('Menssagem'), 'from', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
                     // Load Commands
			switch(command) {
                                case 'help':
                                case 'menu':
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[WARN]','red'), 'Comando não registado de', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
